...
class Manager:
    def __init__(self, nb_inputs, nb_outputs, innov: InovationsTracker):
        ...
        self.outputs_node = []
        self.connections = []
        self.activations = ["sigmoid", "relu", "tanh", "sin", "gauss", "identity", "clamped", "inv", "log", "exp", "abs", "hat", "square", "cube", "softplus"]
        self.aggregations = ["sum", "max", "min", "mul", "avg", "median", "maxabs", "product"]
    def create_genome_initial(self):
        for i in range(self.nb_inputs):
            node = NodeGene(i, "input", random.choice(self.activations), random.uniform(-1, 1), random.choice(self.aggregations))
            self.inputs_node.append(node)
            
        for i in range(self.nb_outputs):
            node_id = self.nb_inputs + i
            node = NodeGene(node_id, "output", random.choice(self.activations), random.uniform(-1, 1), random.choice(self.aggregations))
        ...
        for i in range(self.nb_inputs):
            for j in range(self.nb_outputs):
                ...
                innov_num = self.innov.get_connection_innovation(in_node_id, out_node_id)
                weight = random.uniform(-1, 1)
                conn = ConnectionGene(innov_num, in_node_id, out_node_id, weight, innov_num)
    ...
    def evolution(population, fitness_scores, speciator: Speciator, innov: InovationsTracker, stagnation_limit: int = 15):
        new_population = []
        ...
        for species in species_list:
            if species.members:
                best_genome = sorted(species.members, key=lambda g: g.fitness, reverse=True)
        ...
        for species in species_list:
            if total_adjusted_fitness > 0:
                offspring_count = int((species.adjusted_fitness / total_adjusted_fitness) * remaining_offspring)
            else:
